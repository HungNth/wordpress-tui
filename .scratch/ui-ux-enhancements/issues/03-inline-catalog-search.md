...
- [x] Unit tests in `internal/tui` verify that `SelectPackagesFlow` completes cleanly without search when `__search__` is unselected, and triggers search when selected.
