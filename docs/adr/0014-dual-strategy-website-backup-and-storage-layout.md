...
3. **Strategy 1: Full Source & Portable Database Dump**:
   ...
   - Relocate the completed `.zip` archive from the temporary directory into `backup_path` under `full_<website-slug>_YYYY-MM-DD_HH-mm-ss.zip`. Relocation uses `os.Rename` when source and destination share a volume, and otherwise falls back to a copy into a temp file inside `backup_path`, flush, close, and rename into place, so the destination archive is never left partially written.
   - **Strict Cleanup Invariant**: Delete the temporary `<slug>.sql` file **strictly after both archive creation AND relocation to `backup_path` succeed**. If zip creation, archive finalization, relocation, or dump deletion fails for any reason, return an explicit error and retain `<slug>.sql` on disk so data is preserved for retry and manual recovery.
